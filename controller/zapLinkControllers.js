...
export const editZapLink = async (req, res) => {
  ...
  try {
    ...
    if (req.body.links !== undefined) {
      ...
      for (const [index, link] of links.entries()) {
        // Destructure link properties, providing default values for 'type' and NOW 'isActive'
        const { id, title, url, type = "Website", icon, linkImagePublicId, isActive } = link; // ADD isActive HERE

        console.log(`\n--- Processing Link ${index} (ID: ${id || 'New Link'}) ---`);
        console.log(`Link Data: Title: "${title}", URL: "${url}", Type: "${type}", IsActive: ${isActive}`); // Log isActive
        console.log(`Frontend sent linkImagePublicId: ${linkImagePublicId}`);
        ...
        let currentLinkImage = prev?.linkImage || ""; // Initialize with existing image URL
        let currentLinkPublicId = prev?.linkImagePublicId || ""; // Initialize with existing public ID
        let currentIsActive = prev?.isActive !== undefined ? prev.isActive : true; // Initialize isActive from prev or default to true for new links

        // If isActive is provided in the current link from the request, use it.
        // Otherwise, it will retain its value from `prev` or the default `true`.
        ...
        processedLinks.push({
          ...
          linkImage: currentLinkImage, // Updated or existing image URL
          linkImagePublicId: currentLinkPublicId, // Updated or existing public ID
          isActive: currentIsActive, // Include the isActive field
        });
      }
